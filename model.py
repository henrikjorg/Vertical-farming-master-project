...
class Model:
    ...
    def model(self, t, y, climate):
        env_state = y[:len(env_states_info)]
        crop_state = y[len(env_states_info):]

        # Update models
        env_derivatives = self.env_model.env_conditions(env_state, crop_state, climate)
        plant_derivatives = self.crop_model.crop_conditions(env_state, crop_state)
