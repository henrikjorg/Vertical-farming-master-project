...
class Plotter:
    def __init__(self, t, y, crop_attrs = None, env_attrs = None):
        plt.ion()
        ...
        if env_attrs is not None:
            for attr_name, data in env_attrs.items():
                fig, ax = plt.subplots(1, 1, figsize=(10, 3))
                line, = ax.plot(days, data, label=attr_name)
                ax.set_title(f'{attr_name} over Time')
                ax.set_xlabel('Time [days]')
                ...
                self.env_attr_figs[attr_name] = fig
                self.env_attr_axes[attr_name] = ax
                self.env_attr_lines[attr_name] = line
        plt.pause(1)

    def update_plot(self, t, y, x_lim, crop_attrs=None, env_attrs = None):
        ...
        if env_attrs is not None:
            days = t / (24 * 3600)
            for attr_name, data in env_attrs.items():
                self.env_attr_lines[attr_name].set_data(days, data)
                ax = self.env_attr_axes[attr_name]
                ax.set_xlim(0, x_lim_days)  # Adjust x_lim to days
                ax.relim()
                ax.autoscale_view()
        plt.pause(0.1)
