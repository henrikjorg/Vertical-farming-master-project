...
def export_biomass_ode_model(Crop,Env, Ts, N_horizon, photoperiod_length, darkperiod_length, min_DLI):
    ...
    x = vertcat(X_ns, X_s, FW_plant, DLI_start_of_day, DLI_lower, Tot_cost, TLI, u_last)
    u = vertcat(PPFD)
    ...
    xdot = vertcat(X_ns_dot, X_s_dot, FW_plant_dot, DLI_start_of_day_dot, DLI_lower_dot, Tot_cost_dot, TLI_dot, u_last_dot)
    ...
    f_expl = vertcat(0,#dX_ns,                               # Non-structural dry weight per m^2
                     0,#dX_s,                                # Structural dry weight per m^2
                     (dX_ns + dX_s) * dw_to_fw,           # Fresh weight of the shoot of one plant
                     0,#(photoperiod-1)*(-1)*PPFD/(Ts * photoperiod_length)-photoperiod*0.001*(DLI_start_of_day),#PPFD/(Ts*N_horizon),                 # Average PPFD per day
                     0,
                     PPFD*energy_price/(N_horizon * Ts),  # Average hourly cost of energy for the prediction horizon
                     PPFD/(Ts * photoperiod_length),  # The average PPFD during light period
                     0)
    f_impl = xdot - f_expl

    model = AcadosModel()
    z = vertcat(z_DLI_end_of_day) # The last z variable UL_123 is recently added
    z_constraints = vertcat((DLI_start_of_day + PPFD/(photoperiod_length)))
    h_end = vertcat(DLI_start_of_day,
                    (DLI_start_of_day-min_DLI))

    print('------------')
    model.f_impl_expr = vertcat(f_impl)
